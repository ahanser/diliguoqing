  ...
  data() {
   ...
   watch:{
    mapCityArr(nl,ol){
      ...
      for(let i=0;i<_this.map.layers.items.length;i++){//清除上一次保留的图层
      ...
      esriLoader
        .loadModules(
          ["esri/Map", "esri/views/MapView",   "esri/geometry/Point",
            ...
            "esri/symbols/SimpleLineSymbol",  "esri/layers/Layer", "dojo/domReady!"],
          options
        )
        .then(
          ([Map, MapView, Point, Polygon, Graphic, GraphicsLayer,SimpleLineSymbol,Layer]) => {
                                  ...
                                  for(var i=0;i<res.data.length;i++){
                                      ...
                                      var polygonGraphic = new Graphic({
                                       ...
                                       if(_this.map.allLayers.items.length>=50){
                                          ...
                                          _this.$store.dispatch("quality/launchallLayerDone", allLayerDone);
                                       }
      ...
      esriLoader
        .loadModules(
          ["esri/Map", "esri/views/MapView",   "esri/geometry/Point",
            "esri/geometry/Polygon",
            "esri/Graphic",
            "esri/symbols/SimpleLineSymbol", "esri/layers/FeatureLayer", "esri/layers/GraphicsLayer","dojo/domReady!"],
          options
        )
        .then(
          ([Map, MapView, Point, Polygon, Graphic, SimpleLineSymbol,FeatureLayer,GraphicsLayer]) => {
            var gl = this.initGoogleLayerClass();
