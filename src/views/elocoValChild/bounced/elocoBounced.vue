<template>
  <div>
    <el-dialog :visible.sync="isEcoloFn" 
                @close="colsoDialog"
               width="40%"
               title="生态类型"
               class="Ecological">
...
export default {
  ...
  methods: {
    ...mapActions("val", ["kindListArr"]),
      ...
      localStorage.setItem("elocoValChildType",JSON.stringify(obj));
      this.$store.dispatch("val/kindListArr", arr1);
    },
    colsoDialog(){//关闭弹框
        let selectScoreType=JSON.parse (localStorage.getItem("elocoValChildType"));
