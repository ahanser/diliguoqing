<template>
    ...
    <el-dialog title="新增角色" :visible.sync="isAdd" width="30%">
        <el-form label-width="100px" :model="addItem">
            <el-form-item label="角色名称：">
                <el-input v-model="addItem.name"></el-input>
            </el-form-item>
            <el-form-item label="角色描述：">
                <el-input v-model="addItem.remark" type="textarea"></el-input>
            </el-form-item>
    ...
    <el-dialog title="编辑角色" :visible.sync="isEdit" width="30%">
        <el-form label-width="100px" :model="editItem">
            <el-form-item label="角色名称：">
                <el-input v-model="editItem.name"  ></el-input>
            </el-form-item>
            <el-form-item label="角色描述：">
                <el-input v-model="editItem.remark" type="textarea"></el-input>
            </el-form-item>
    ...
    <el-dialog title="分配权限" :visible.sync="isAssign" width="50%">
        <el-collapse  v-model="activeNames" >
            <el-collapse-item  :title="item.name"  v-for="(item,index) in  permiss" v-if="item.perms!=null" :name="index" :key="index">
                    
               <el-checkbox v-for="per in item.perms" :label="per.id" :key="per.id"  v-model="checkboxGroup4" >{{per.name}}</el-checkbox>
...
export default {
  ...
  methods: {
        ...
        function flatten(data, keys, level = 0) {
          return data.reduce(
            (arr, x) => [
              ...
              ...flatten(x.sub || [], keys, level + 1)
            ],
            []
          );
        }

        this.permiss = flatten(arr, [
          "id",
          "name",
          "parentId",
          "path",
    ...
    async addroleConfirm() {
      let result = await this.$httpPost("role/add", this.addItem);
    ...
    async editroleConfirm() {
      let result = await this.$httpPost("role/update", this.editItem);
